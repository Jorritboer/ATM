...
class BAM_QMDP:
    ...
    def __init__(
        self, env: AM_ENV, eta=0.00, nmbr_particles=100, offline_training_steps=0
    ):
        ...
        # Meta-variables:
        self.eta = (
            eta  # Chance of picking a non-greedy action (should be called epsilon...)
        )
        self.nmbr_particles = (
        ...
        self.initPrior = (
            1 / self.StateSize
        )  # Initial alpha-values, as used in the prior for T
    ...
    def init_run_variables(self):
        ...
        self.QTableUnbiased = np.zeros(
            (self.StateSize, self.ActionSize), dtype=np.longfloat
        )  # Q-table based solely on experience (unbiased)
    ...
    def run_episode(self):
        ...
        while not self.is_done:
            ...
            measure = (
                ...
                or (TransSupport < self.NmbrOptimiticTries)
...
Unbiased QTable: {}            
            """.format(
                    self.alpha, self.QTable, self.QTableRewards, self.QTableUnbiased
                )
            )
        if get_full_results:
    ...
    def get_action(self, S):
        ...
        return int(
            np.random.choice(np.where(np.isclose(thisQ, thisQMax))[0])
        )  # randomize tiebreaks
    ...
    def sample_T(self, s, action, nmbr=1):
        ...
        if self.use_exp:
            if self.alpha_sum[s, action] > 8:
                return (self.alpha[s, action] - self.initPrior) / (
                    self.alpha_sum[s, action] - (self.StateSize * self.initPrior)
                )
    ...
    def guess_next_state(self, S, action):
        ...
        if np.sum(probDist) == 0:
            probDist, states = np.ones(self.StateSize) / self.StateSize, np.arange(
                self.StateSize
            )
            ...
            if not (s1 == self.doneState):
                ...
                if thisAlpha >= self.NmbrOptimiticTries and self.optimism_type != "UCB":
                    ...
                    if self.optimism_type == "RMAX+":
                        self.QTable[s1, action] = totQ + max(0, 1 - totQ) * (
                            (self.NmbrOptimiticTries - thisAlpha)
                            / self.NmbrOptimiticTries
                        )
                    elif self.optimism_type == "UCB":
                        optTerm = np.sqrt(
                            2 * np.log10(self.totalSteps + 2) / (self.QCounter + 1)
                        )  # How do we do this +2 cleanly?
                        self.QTableUnbiased[s1, action] = totQ
